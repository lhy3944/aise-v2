...
export default function ProjectsPage() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : '프로젝트 목록을 불러올 수 없습니다.';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  async function handleCreate(data: ProjectCreate) {
    ...
    try {
      ...
      const message = err instanceof ApiError ? err.message : '프로젝트 생성에 실패했습니다.';
      setError(message);
    } finally {
      setCreating(false);
    }
        ...
        {isLoading ? (
          ...
          search ? (
            <div className='animate-in fade-in flex min-h-[calc(100vh-14rem)] flex-col items-center justify-center text-center duration-300'>
              ...
              <h2 className='text-fg-primary text-base font-medium'>검색 결과가 없습니다</h2>
              <p className='text-fg-secondary text-sm'>다른 검색어로 시도해 보세요.</p>
            </div>
          ) : (
            <div className='animate-in fade-in flex min-h-[calc(100vh-14rem)] items-center justify-center'>
              ...
              >
                ...
                <h2 className='text-fg-primary text-base font-medium'>프로젝트가 없습니다</h2>
                <p className='text-fg-secondary text-sm'>새 프로젝트를 만들어 시작하세요.</p>
              </Button>
            </div>
          )
        ) : viewMode === 'card' ? (
