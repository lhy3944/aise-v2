...
| AI Framework | Deep Agents (LangGraph 기반), 단순 작업은 직접 API 호출 |
...
프론트엔드 코드 작성 전 `frontend/docs/guides/` 하위 가이드를 참조할 것:
- `code-conventions.md` — 디렉토리 구조, 파일 배치 규칙, 네이밍
- `component-patterns.md` — 컴포넌트 설계 패턴 (Link 내부 버튼, 높이 일관성 등)
- `design-tokens.md` — AISE+ 시맨틱 토큰 체계 (색상, 타이포, 간격)
- `api-patterns.md` — API 연동 표준 (useFetch, useMutation, 에러 처리)
